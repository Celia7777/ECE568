...
void update_info(connection* C, std::string buyer_id, std::string seller_id, std::string symbol, double amount, double price, work& W3){
    ...
    find_account_buyer << "SELECT BALANCE FROM ACCOUNT WHERE ACCOUNT_ID=" << W3.quote(buyer_id) << ";";
    ...
    find_account_seller << "SELECT BALANCE FROM ACCOUNT WHERE ACCOUNT_ID=" << W3.quote(seller_id) << ";";
    ...
    find_position_buyer << "SELECT AMOUNT FROM POSITION WHERE ACCOUNT_ID=" << W3.quote(buyer_id) << " AND SYMBOL=" << W3.quote(symbol) << ";";
    ...
    find_position_seller << "SELECT AMOUNT FROM POSITION WHERE ACCOUNT_ID=" << W3.quote(seller_id) << " AND SYMBOL=" << W3.quote(symbol) << ";";
...
int MatchOrder(connection* C, std::string account_id, int order_id, std::string symbol, double amount, double price){
    ...
    if(amount < 0){
        ...
        for(auto it = Order.begin(); it != Order.end(); it++){
            ...
            if (abs(it[3].as<double>()) == abs(res_amount)){
                ...
                if (checkValid(C, it[1].as<std::string>(), account_id, symbol, abs(res_amount), it[4].as<double>(), W)){
                    return DB_Fail;
                }
                // work W(*C);
                //delete buy order
                ...
                // W.commit();
                update_info(C, it[1].as<std::string>(), account_id, symbol, abs(res_amount), it[4].as<double>(), W);
                // return DB_SUCCESS;
            }
            //buy order shares < sell amount
            else if(abs(it[3].as<double>()) < abs(res_amount)){
                //check whether one order is valid
                if (checkValid(C, it[1].as<std::string>(), account_id, symbol, abs(it[3].as<double>()), it[4].as<double>(), W)){
                    return DB_Fail;
                }
                // work W(*C);
                time_t current_time = time(NULL);
                ...
                // W.commit();
                update_info(C, it[1].as<std::string>(), account_id, symbol, abs(it[3].as<double>()), it[4].as<double>(), W);
            }
    ...
    //amount > 0
        ...
        for(auto it = Order.begin(); it != Order.end(); it++){
            ...
            if (abs(it[3].as<double>()) == abs(res_amount)){
                //check whether one order is valid
                if (checkValid(C, account_id, it[1].as<std::string>(), symbol, abs(res_amount), price, W)){
                    return DB_Fail;
                }
                // work W(*C);
                //delete buy order
                ...
                W.exec(insert_executed_buy.str());
                // W.commit();
                update_info(C, account_id, it[1].as<std::string>(), symbol, abs(res_amount), price, W);
                // amount =0;
            }
            //buy order shares < sell amount
            else if(abs(it[3].as<double>()) > abs(res_amount)){
            ...
            //buy order share > sell amount
                ...
                //check whether one order is valid
                if (checkValid(C, account_id, it[1].as<std::string>(), symbol, abs(it[3].as<double>()), price, W)){
                    return DB_Fail;
                }
                // work W(*C);
                time_t current_time = time(NULL);
                ...
                // W.commit();
                update_info(C, account_id, it[1].as<std::string>(), symbol, abs(it[3].as<double>()), price, W);

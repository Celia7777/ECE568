    ...
    if(amount < 0){
        ...
        for(auto it = Order.begin(); it != Order.end(); it++){
            ...
            //buy order shares < sell amount
                ...
                //add left sell amount into OPEN table 
                std::stringstream insert_open_sell;
                insert_open_sell << "INSERT INTO OPEN (ACCOUNT_ID, SYMBOL, AMOUNT, PRICE, TIME) VALUES (" <<W.quote(account_id)<< W.quote(symbol)<<", "<< (it[3].as<double>() + amount) <<", "<< price<<", "<< current_time << ";";
...
void query_order(connection* C, std::string account_id, int order_id, std::vector<open_order>& open, std::vector<canceled_order>& cancel, std::vector<executed_order>& executed){
    ...
    result R_open(W.exec(find_open.str()));
    ...
    result R_executed(W.exec(find_executed.str()));
    ...
    result R_cancel(W.exec(find_cancel.str()));
